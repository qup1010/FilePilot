    ...
    <div className="space-y-2.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          title="一键整理功能说明"
          className="group flex h-5 w-5 items-center justify-center rounded-full bg-on-surface/5 text-on-surface-variant/70 hover:bg-primary/10 hover:text-primary transition-colors focus:outline-none"
        >
        ...
        <span className="text-[11px] font-bold text-on-surface-variant/50">
          按目标规则全自动分类归档，不确定项保留原位
        </span>
      </div>

      {/* 功能说明展开卡片 */}
      {showHelpCard ? (
        <div className="relative rounded-xl border border-primary/20 bg-surface-container-lowest p-3.5 shadow-md backdrop-blur-sm animate-in fade-in-50 zoom-in-95 space-y-2.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-[12px]">
            <div className="rounded-lg bg-surface-container/40 p-2.5 border border-on-surface/6 space-y-1">
      ...
      {/* 错误提示 */}
      {error ? (
        <div className="flex items-center gap-2 rounded-lg border border-error/30 bg-error/5 px-3 py-2 text-[12px] font-semibold text-error">
      ...
      {/* 统一高颜值单行控制面板 */}
      <div className="relative flex flex-wrap items-center gap-2 rounded-xl border border-on-surface/8 bg-surface-container-lowest p-2.5 shadow-sm transition-all hover:border-on-surface/15">
        {/* 选择源目录 Pill */}
        <button
          type="button"
          onClick={() => void handlePickSource()}
          title={sourcePath || "点击选择要整理的源文件夹"}
          className="group flex max-w-[260px] items-center gap-2 rounded-lg border border-on-surface/10 bg-surface/80 px-3 py-2 text-left text-[12px] font-bold text-on-surface transition-all hover:border-primary/40 hover:bg-surface active:scale-[0.98]"
        >
        ...
        {/* 箭号分隔 */}
        <ChevronRight className="h-4 w-4 shrink-0 text-on-surface-variant/30 hidden sm:block" aria-hidden />

        {/* 选择分类规则 Dropdown Pill */}
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            onClick={() => setIsDropdownOpen((prev) => !prev)}
            className="flex items-center gap-2 rounded-lg border border-on-surface/10 bg-surface/80 px-3 py-2 text-left text-[12px] font-bold text-on-surface transition-all hover:border-primary/40 hover:bg-surface active:scale-[0.98]"
          >
            <span className="text-[11px] font-medium text-on-surface-variant/50">规则:</span>
            <span className="max-w-[140px] truncate font-bold text-on-surface">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-[12px] font-black text-on-primary shadow-sm transition-all hover:bg-primary/90 hover:shadow active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
        >
          {launching ? (
            <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
          ) : (
            <Zap className="h-4 w-4 fill-on-primary/20" aria-hidden />
          )}
          <span>{launching ? "正在开启..." : "一键整理"}</span>
        </button>
      </div>
    </div>
  );
