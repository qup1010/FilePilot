...
export function ErrorAlert({ title = "系统指令异常中断", message, onClose }: ErrorAlertProps) {
  return (
    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex gap-6 p-1">
      ...
      <div className="bg-error-container/[0.03] text-error px-6 py-5 rounded-[24px] border border-error/10 flex-1 space-y-1.5 shadow-sm relative group">
        ...
        {onClose && (
          <button 
            onClick={onClose}
            className="absolute right-4 top-4 opacity-0 group-hover:opacity-40 hover:opacity-100 transition-opacity"
          >
            关闭
          </button>
        )}
