...
class ImageDescriberTests(unittest.TestCase):
    ...
    def test_describe_image_uses_isolated_messages_and_returns_short_summary(self):
        ...
        self.assertIn("帮助理解图片用途", kwargs["messages"][0]["content"])
        self.assertIn("图片类型：", str(kwargs["messages"][1]["content"]))
        self.assertIn("主体内容：", str(kwargs["messages"][1]["content"]))
        self.assertIn("可读文字：", str(kwargs["messages"][1]["content"]))
        self.assertIn("关键线索：", str(kwargs["messages"][1]["content"]))
        self.assertIn("用途画像：", str(kwargs["messages"][1]["content"]))
        self.assertIn("限制：", str(kwargs["messages"][1]["content"]))
