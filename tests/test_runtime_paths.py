...
from file_organizer.shared import constants
from file_organizer.shared import config as config_module


class RuntimePathTests(unittest.TestCase):
    def test_resolve_project_root_prefers_environment_override(self):
        previous = os.environ.get("FILE_ORGANIZER_PROJECT_ROOT")
        temp_dir = Path("output/test_runtime_root").resolve()
        temp_dir.mkdir(parents=True, exist_ok=True)
        os.environ["FILE_ORGANIZER_PROJECT_ROOT"] = str(temp_dir)
        self.assertEqual(constants.resolve_project_root(), temp_dir)

        if previous is None:
            os.environ.pop("FILE_ORGANIZER_PROJECT_ROOT", None)
        else:
            os.environ["FILE_ORGANIZER_PROJECT_ROOT"] = previous
