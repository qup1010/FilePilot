...
class SessionStore:
    ...
    def mark_abandoned(self, session_id: str) -> None:
        with self._write_lock:
            ...
            if session is None:
                return
            self.mark_session_abandoned(session)
    ...
    def mark_session_abandoned(self, session: OrganizerSession) -> None:
        with self._write_lock:
            ...
            latest_index = self._read_latest_index()
            if latest_index.get(session.target_dir) == session.session_id:
            ...
            if payload.get("owner_session_id") == session.session_id and lock_path.exists():
                lock_path.unlink()
