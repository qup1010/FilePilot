...
from file_pilot.shared.config import SESSIONS_DIR, SPOOF_HEADERS
...
def create_app(service: OrganizerSessionService | None = None) -> FastAPI:
    ...
    @app.get("/api/icon-workbench/sessions")
    def list_icon_workbench_sessions():
        return app.state.icon_workbench_service.list_sessions(limit=20)
    ...
    @app.delete("/api/icon-workbench/sessions/{session_id}")
    def delete_icon_workbench_session(session_id: str):
        try:
            return app.state.icon_workbench_service.delete_session(session_id)
        except FileNotFoundError:
            raise HTTPException(status_code=404, detail="ICON_SESSION_NOT_FOUND")
